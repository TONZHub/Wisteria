...
Wisteria is designed first for people who already live with PMDD and are entering perimenopause, when cycles and familiar warning signs may become less predictable. Its calendar-agnostic design can also remain useful when cycle timing is irregular for other reasons, including PCOS, without asking the person to identify a phase or asking Wisteria to diagnose a condition. Some days, the hardest part of self-reflection is finding a full sentence. Wisteria accepts a number, a tap, or an everyday phrase such as “I feel off,” turns it into one gentle daily record, and stays present for a follow-up without accidentally logging the conversation twice. Over time, its user-triggered Night Shift notices recurring heavy-to-off stretches and prepares a plain-language brief with visible confidence and evidence.
...
- Early feedback about PCOS highlighted a broader design lesson: calendar-agnostic tracking can stay useful for irregular cycles without turning the app into a diagnostic tool.
